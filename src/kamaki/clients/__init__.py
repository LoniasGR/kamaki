...
from http.client import ResponseNotReady, HTTPException
...
class ClientError(Exception):
    def __init__(self, message, status=0, details=None):
        log.debug("ClientError: msg[%s], sts[%s], dtl[%s]" % (message, status, details))
        try:
            message += "" if message and message[-1] == "\n" else "\n"
            serv_stat, sep, new_msg = message.partition("{")
            new_msg = sep + new_msg[: -1 if new_msg.endswith("\n") else 0]
            json_msg = loads(new_msg)
            key = list(json_msg.keys())[0]
...
class RequestManager(Logged):
    ...
    def __init__(self, method, url, path, data=None, headers={}, params={}):
        method = method.upper()
    ...
    def dump_log(self):
        ...
        sendlog.log(
            DEBUGV,
            "%s %s://%s%s%s" % (self.method, self.scheme, self.netloc, self.path, plog),
        )
        ...
        if self.data:
            ...
            if self.LOG_DATA:
                sendlog.log(
                    DEBUGV,
                    utils.escape_ctrl_chars(
                        self.data.replace(self._token, "...")
                        if self._token
                        else self.data
                    ),
                )
    ...
    def _encode_headers(self):
        ...
        for k, v in list(self.headers.items()):
            ...
            ) or any([key.startswith(p) for p in self._header_prefices])
...
class ResponseManager(Logged):
    ...
    def _get_headers_to_decode(self, headers):
        ...
        return encodable + list(filter(has_prefix, keys.difference(encodable)))
        ...
        for retries in range(1, self.CONNECTION_TRY_LIMIT + 1):
            try:
                with https.PooledHTTPConnection(
                    self.request.netloc, self.request.scheme, **pool_kw
                ) as connection:
                    ...
                    recvlog.log(
                        DEBUGV, "%d %s%s" % (self.status_code, self.status, plog)
                    )
                    self._headers = dict()
                    ...
                    for k, v in r_headers:
                        self._headers[k] = (
                            unquote(v) if (k.lower()) in enc_headers else v
                        )
...
class SilentEvent(Thread):
    ...
    def run(self):
        try:
            ...
            recvlog.debug(
                "Thread %s got exception %s\n<%s %s" % (self, type(e), estatus, e)
            )
            self._exception = e
...
class Client(Logged):
    ...
    def __init__(self, endpoint_url, token):
        ...
        if not self.api_version:
            ...
            log.info(
                "Endpoint does not provide API version. Using default %s"
                "API version: %s",
                self.service_type,
                self.DEFAULT_API_VERSION,
            )
        ...
        if https.HTTPSClientAuthConnection.cafile is None:
            try:
                from kamaki import defaults

                https.HTTPSClientAuthConnection.cafile = getattr(
    ...
    def _watch_thread_limit(self, threadlist):
        ...
        if (
            self._elapsed_old
            and self._elapsed_old >= self._elapsed_new
            and (self._thread_limit < self.MAX_THREADS)
        ):
    ...
    def request(
        self, method, path, async_headers=dict(), async_params=dict(), **kwargs
    ):
        ...
        """
        assert isinstance(method, str) or isinstance(method, str)
        assert method
        assert isinstance(path, str) or isinstance(path, str)
        try:
...
class Waiter(object):
    ...
    def wait_until(
        self, item_id, target_status, get_status, delay=1, max_wait=100, wait_cb=None
    ):
        return self._wait(
            item_id,
            target_status,
